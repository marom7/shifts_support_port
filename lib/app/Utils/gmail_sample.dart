...
// שלב 1: חיפוש כל ההודעות
  ...
  final sinceDate = DateFormat('dd-MMM-yyyy').format(fiveDaysAgo); // פורמט IMAP נכון
    final search = await client.searchMessages(searchCriteria: 'SINCE $sinceDate');
  ...
  for (var i = envRes.messages.length - 1; i >= 0; i--) {
    ...
    //final html = htmlRes.messages[i].decodeTextHtmlPart();
  ...
  //  print('מאת:  $from');
   ...
   // print('---------------------------');
     print('----------------------');
      if (from.contains("app@ashdodport.co.il")) {
